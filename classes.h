...
enum parameters {ELEMENT_LENGTH,T_CTE,K_CTE,LAMBDA_CTE,U_CTE,FI_CTE,ALFA_CTE,DELTA_CTE,N_CTE};
...
class mesh{
    ...
    public:
        void setParameters(float l,float t,float k,float lambda,float u,float fi,float alfa,float delta,float n){
            parameters[ELEMENT_LENGTH]=l;
            parameters[T_CTE]=t;
            parameters[K_CTE]=k;
            parameters[LAMBDA_CTE]=lambda;
            parameters[U_CTE]=u;
            parameters[FI_CTE]=fi;
            parameters[ALFA_CTE]=alfa;
            parameters[DELTA_CTE]=delta;
            parameters[N_CTE]=n;
        }
