...
void createLocalB(Matrix &B,mesh m){
    ...
    float k = m.getParameter(K_CTE);
    B.at(0).at(0) += k/l;      B.at(0).at(1) += -k/l;
    B.at(1).at(0) += -k/l;     B.at(1).at(1) += k/l;
}

void createLocalB_Copy(Matrix &H,mesh m){
    float l = m.getParameter(ELEMENT_LENGTH);
    float u = m.getParameter(U_CTE);
    H.at(0).at(0) += u/l;      H.at(0).at(1) += -u/l;
    H.at(1).at(0) += -u/l;     H.at(1).at(1) += u/l;
}

void createLocalC(Matrix &C,mesh m){
    float lambda = m.getParameter(LAMBDA_CTE);
    C.at(0).at(0) += -lambda/(3);    C.at(0).at(1) += lambda/(3);
    C.at(1).at(0) += -lambda/(3);    C.at(1).at(1) += lambda/(3);
}

void createLocalD(Matrix &D,mesh m){
	float alfa = m.getParameter(ALFA_CTE);
    D.at(0).at(0) += -(3*alfa)/2;  D.at(0).at(1) += (3*alfa)/2;
    D.at(1).at(0) += -(3*alfa)/2;  D.at(1).at(1) += (3*alfa)/2;
}

void createLocalD_Copy(Matrix &E,mesh m){
	float delta = m.getParameter(DELTA_CTE);
...
Matrix createLocalK(int element,mesh &m){
    ...
    createLocalC(C,m);
    createLocalD(D,m);
	createLocalB_Copy(H,m);
	createLocalD_Copy(E,m);
...
Vector createLocalb(int element,mesh &m){
    ...
    float fi = m.getParameter(FI_CTE); 
	float l = m.getParameter(ELEMENT_LENGTH);
    float n = m.getParameter(N_CTE);
    b.push_back(fi*l/2); 
    b.push_back(fi*l/2); 
    b.push_back(n*l/2); 
    b.push_back(n*l/2);
